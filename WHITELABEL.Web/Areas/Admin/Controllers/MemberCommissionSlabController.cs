    ...
    public class MemberCommissionSlabController : AdminBaseController
    {
        ...
        private IGrid<TBL_WHITE_LEVEL_COMMISSION_SLAB> CreateExportableGrid()
        {
            ...
            grid.Columns.Add(model => model.SLN).Titled("").Encoded(false)
                .RenderedAs(model => "<a href='" + @Url.Action("GenerateCommissionSlab", "MemberCommissionSlab", new { area = "Admin", SlabId = Encrypt.EncryptMe(model.SLN.ToString()) }) + "' class='btn btn-primary btn-xs'>Edit</a>");
