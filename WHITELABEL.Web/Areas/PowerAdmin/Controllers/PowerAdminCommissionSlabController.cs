    ...
    public class PowerAdminCommissionSlabController : PoweradminbaseController
    {
        ...
        private IGrid<TBL_WHITE_LEVEL_COMMISSION_SLAB> CreateExportableGrid()
        {
            ...
            grid.Columns.Add(model => model.SLN).Titled("").Encoded(false)
                .RenderedAs(model => "<a href='" + @Url.Action("AddCommissionSlab", "PowerAdminCommissionSlab", new { area = "PowerAdmin", SlabId = Encrypt.EncryptMe(model.SLN.ToString()) }) + "' class='btn btn-primary btn-xs'>Edit</a>");            
