...
namespace WHITELABEL.Web.Areas.PowerAdmin.Controllers
{
    ...
    public class PowerAdminBankDetailsController : PoweradminbaseController
    {
        ...
        private IGrid<TBL_SETTINGS_BANK_DETAILS> CreateExportableGrid()
        {
            ...
            grid.Columns.Add(model => model.SL_NO).Titled("").Encoded(false).Filterable(false).Sortable(false)
                .RenderedAs(model => "<a href='" + @Url.Action("BankDetails", "PowerAdminBankDetails", new { area = "PowerAdmin", Bankid = Encrypt.EncryptMe(model.SL_NO.ToString()) }) + "' class='btn btn-primary btn-xs'>Edit</a>");
            grid.Columns.Add(model => model.SL_NO).Titled("").Encoded(false).Filterable(false).Sortable(false)
